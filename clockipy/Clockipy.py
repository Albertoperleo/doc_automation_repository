...
class Clockipy:
    ...
    def request(self, url, options):
        ...
        for key in options:
            if type(value := options.get(key)) == str:
                path.replace("{%s}" % key, value)

        result = requests.request(method, "%s%s" % (options.get("base_url") or self.base_url, path),
                                  headers={**self.headers, **(options.get("headers") or {})}, json=options.get("body"),
